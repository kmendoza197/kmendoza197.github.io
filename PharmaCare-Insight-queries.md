...
-- What are the brand names of top 5 drugs with the most adverse effects?
SELECT "brand_name"
FROM "drugs"
WHERE "id" IN (
    SELECT "drug_id"
...
JOIN "adverse_drug_reactions" ON "prescriptions"."drug_id" = "adverse_drug_reactions"."drug_id" AND "prescriptions"."patient_id" = "adverse_drug_reactions"."patient_id"
WHERE "condition_treated" = 'arthritis'
AND "reaction_type" = 'allergic reaction'
;

-- Updating when a patient has recovered from an adverse drug reaction

UPDATE "adverse_drug_reactions"
